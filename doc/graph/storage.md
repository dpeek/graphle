...
- `../../lib/app/src/web/lib/graph-authority-sql-storage.ts` for the stable
  persisted-authority SQL storage seam for durable reads, writes, and schema
  bootstrap
- `../../lib/app/src/web/lib/graph-authority-sql-startup.ts` for startup metadata
  hydration, retained-history recovery classification, and stable startup
  diagnostics assembly
- `../../lib/app/src/web/lib/graph-authority-internal-routes.ts` for the web-only
  session-principal and bearer-share lookup route parsing plus error mapping
- `../../lib/app/src/web/lib/graph-authority-do.ts` for Durable Object bootstrap,
...
- file-backed persisted authorities normalize legacy write histories that lack
  ...
  origin
- coverage in `../../lib/app/src/web/lib/graph-authority-do.test.ts` exercises
...
- the web Durable Object authority path in
  `../../lib/app/src/web/lib/graph-authority-do.ts` no longer uses
