...
- `src/graph/runtime/client.ts` and `src/graph/runtime/client-query.ts`: keep
  typed local entity and neighborhood reads, but separate them from
  projection-backed collection queries
