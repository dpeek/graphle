...
Each experiment lives under `../../src/app/experiments/<name>/`.
...
- `../../src/app/experiments/company/`: company schema plus company/query/relationship
  proof routes
- `../../src/app/experiments/outliner/`: block schema plus outliner route
- `../../src/app/experiments/env-vars/`: env-var and secret-ref schema plus settings
  route
- `../../src/app/experiments/explorer/`: route-only graph tooling slice
- `../../src/app/experiments/workspace/`: promoted workspace schema wiring plus the
...
- `app:` is the bucket for the current experiment and domain slices exposed
  through `src/app/experiments/*`, including the current company/person,
...
the canonical modules from `@io/core/graph/schema/app/*` and stay focused on
...
- `../../src/app/experiments/contracts.ts`: typed experiment graph and web contracts
- `../../src/app/experiments/graph.ts`: registry that merges experiment schema and
  seed hooks
- `../../src/app/experiments/web.ts`: registry that merges experiment routes
- `../../src/app/graph/app.ts`: app namespace composed from registered experiment
  schema
- `../../src/app/graph/example-data.ts`: example runtime seeding composed from
  registered experiment hooks
- `../../src/app/web/runtime.tsx`: shared synced runtime bootstrap
- `../../src/app/web/app-shell.tsx`: shared shell, navigation, and canonical route
  behavior
...
1. Move the canonical type modules into `src/graph/schema/app/<slice>/` with
...
1. Create `../../src/app/experiments/<name>/graph.ts` if the slice owns schema or seed
   wiring.
   Promoted reusable schema should be imported from `@io/core/graph/schema/app/*`
   rather than redefined there.
2. Create `../../src/app/experiments/<name>/web.ts` if the slice owns routed UI.
3. Add the graph definition to `../../src/app/experiments/graph.ts` and the web
   definition to `../../src/app/experiments/web.ts`.
