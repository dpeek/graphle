...
- turn the current issue into a landed local change on `main`
- keep the change narrowly scoped to the issue
- leave the dedicated issue worktree in a reviewable state
...
- stay inside the current issue worktree and do not disturb unrelated local changes
...
- if landing onto local `main` would be unsafe or require conflict resolution, stop and report the blocker
...
6. land the change onto local `main` only if it is safe
...
- validation result
- commit SHA
- landing SHA on `main`, or the blocker that prevented landing
