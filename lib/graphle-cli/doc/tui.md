...
description: "Current graph-backed workflow TUI owned by @dpeek/graphle-cli."
...
- keep terminal workflow screen composition, shell chrome, and `graphle tui`
...
- keep graph source resolution, startup hydration sequencing, and initial
  ...
  `../src/tui/startup.ts`
- keep reusable host-neutral graph React bindings in `@dpeek/graphle-react`
- keep reusable workflow projection contracts and read helpers in
  `@dpeek/graphle-module-workflow` and the owning graph runtime packages
- keep the retained Linear or session monitor in `../src/agent/tui/*` until
  workflow views replace it

## Entry point

- `graphle tui [entrypointPath]`

## Current behavior

- loads repo workflow config through the shared `graphle.ts` and `graphle.md`
...
The default local graph served from `http://graphle.localhost:1355/` seeds one
workflow project, repository, branch, commit, and running session so a fresh
local `graphle tui` can hydrate without extra graph setup.
...
- workflow entrypoint loading stays shared with `graphle agent ...`
- graph source precedence is `--graph-url`, then `graphle.ts -> tui.graph.url`,
  then `http://graphle.localhost:1355/`
- sync scope is fixed to the workflow review module scope
- initial project resolution is `--project`, then
  `graphle.ts -> tui.initialScope.project`, then infer the one visible
  `WorkflowProject`
- initial branch resolution is `--branch`, then
  `graphle.ts -> tui.initialScope.branch`, then the first branch-board row in the
  resolved project
