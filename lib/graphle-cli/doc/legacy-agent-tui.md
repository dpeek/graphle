...
description: "Current retained-session monitor owned by @dpeek/graphle-cli for `graphle agent tui ...`."
...
- you are changing `graphle agent tui ...`, `graphle agent tail ...`, retained attach or
...
- `graphle agent tui [entrypointPath] [--once]`: live supervisor plus worker view
- `graphle agent tui attach <issue> [entrypointPath]`: follow one retained issue
  until its worker session reaches a terminal phase
- `graphle agent tui replay <issue> [entrypointPath] [--delay-ms <ms>]`: replay one
  retained issue with a default `40ms` event delay
- `graphle agent tail <issue> [entrypointPath]`: raw tail of the retained issue log
