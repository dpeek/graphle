---
name: CLI roadmap
description: "Future operator-runtime direction for @dpeek/graphle-cli."
...
- you are deciding future direction for `@dpeek/graphle-cli`
...
`@dpeek/graphle-cli` already ships:

- the `graphle` binary and top-level task dispatch
...
`@dpeek/graphle-cli` currently carries both the graph-backed workflow shell and the legacy
