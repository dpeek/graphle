---
name: CLI graph MCP
description: "Current stdio graph MCP surface owned by @dpeek/graphle-cli."
...
- you are changing `graphle mcp graph`
...
`@dpeek/graphle-cli` ships a stdio MCP server that talks to the existing graph HTTP
...
- `--url`: `@dpeek/graphle-client` default HTTP graph URL
