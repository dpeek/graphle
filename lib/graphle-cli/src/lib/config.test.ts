...
test("loadIoConfig prefers graphle.ts and resolves env-backed values", async () => {
...
`,
  ...
  try {
    ...
    expect(result.value.sourcePath).toBe(resolve(root, "graphle.ts"));
...
test("loadIoConfig falls back to graphle.json and normalizes legacy install config", async () => {
  const root = await mkdtemp(resolve(tmpdir(), "graphle-config-"));
