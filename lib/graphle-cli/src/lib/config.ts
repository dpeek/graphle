...
export const IO_JSON_FILE = "graphle.json";
export const IO_TS_FILE = "graphle.ts";
...
export async function loadIoConfig({
  ...
  if (!sourceKind) {
    return invalidResult(
      `Unsupported graphle config path: ${absolutePath}. Expected ${IO_TS_FILE} or ${IO_JSON_FILE}.`,
      absolutePath,
    );
  }
  if (!existsSync(absolutePath)) {
    return invalidResult(`Missing graphle config entrypoint: ${absolutePath}`, absolutePath);
  }
