...
test("loadWorkflowFile loads graphle.ts and graphle.md by default", async () => {
  ...
  await writeIoTsConfig(root, {
    ...
    context: {
      docs: {
        "project.overview": "./graphle/context/overview.md",
      },
      profiles: {
        execute: {
          include: ["builtin:graphle.agent.execute.default", "project.overview"],
        },
      },
    },
    install: { brews: ["bat"] },
  ...
  await writeFile(resolve(root, "graphle.md"), "Graphle {{ issue.identifier }}\n");
  await writeFile(resolve(root, "graphle", "context", "overview.md"), "PROJECT OVERVIEW\n");
  ...
  try {
    ...
    expect(result.value.entrypoint.kind).toBe("graphle");
    expect(result.value.entrypoint.configPath).toBe(resolve(root, "graphle.ts"));
    expect(result.value.entrypoint.promptPath).toBe(resolve(root, "graphle.md"));
    expect(result.value.entrypointContent).toBe("Graphle {{ issue.identifier }}");
...
test("loadWorkflowFile rejects legacy entrypoint paths", async () => {
  ...
  await writeFile(resolve(root, "graphle.md"), "Graphle {{ issue.identifier }}\n");
  await writeFile(resolve(root, "graphle.json"), "{}\n");
  await writeFile(resolve(root, "WORKFLOW.md"), "legacy\n");

  try {
    const ioJsonResult = await loadWorkflowFile("graphle.json", root);
    ...
    expect(ioJsonResult.errors).toEqual([
      {
        message: "Unsupported entrypoint path: graphle.json. Expected graphle.ts or graphle.md.",
        path: "$",
      },
    ]);
    ...
    expect(workflowResult.errors).toEqual([
      {
        message: "Unsupported entrypoint path: WORKFLOW.md. Expected graphle.ts or graphle.md.",
        path: "$",
      },
    ]);
  } finally {
...
test("loadWorkflowFile resolves builtin override paths from graphle.ts", async () => {
  const root = await mkdtemp(resolve(tmpdir(), "workflow-"));
  process.env.LINEAR_API_KEY = "linear-token";
  ...
  await writeIoTsConfig(root, {
    context: {
      overrides: {
        "builtin:graphle.core.validation": "./graphle/context/validation.md",
      },
    },
    tracker: {
      apiKey: "$LINEAR_API_KEY",
  ...
  try {
    ...
    expect(result.value.context.overrides).toEqual({
      "builtin:graphle.core.validation": resolve(root, "graphle", "context", "validation.md"),
    });
  } finally {
    await rm(root, { force: true, recursive: true });
  }
...
test("loadWorkflowFile resolves registered docs and merges default context profiles", async () => {
  ...
  await writeIoTsConfig(root, {
    context: {
      docs: {
        "project.architecture": "./graphle/context/architecture.md",
      },
      profiles: {
        backlog: {
          include: ["builtin:graphle.agent.backlog.default", "project.architecture"],
        },
      },
    },
    tracker: {
  ...
  await writeFile(resolve(root, "graphle.md"), "Graphle {{ issue.identifier }}\n");
  await writeFile(resolve(root, "graphle", "context", "architecture.md"), "ARCHITECTURE\n");
...
test("loadWorkflowFile preserves profile entrypoint opt-out", async () => {
  ...
  await writeIoTsConfig(root, {
    context: {
      profiles: {
        execute: {
          include: ["builtin:graphle.agent.execute.default"],
...
test("loadWorkflowFile parses issue routing defaults and normalized rules from graphle.ts", async () => {
  const root = await mkdtemp(resolve(tmpdir(), "workflow-"));
