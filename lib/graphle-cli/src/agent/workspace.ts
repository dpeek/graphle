...
import { createLogger, type Logger } from "@dpeek/utils";
...
function toStreamBranchName(issueIdentifier: string) {
  return `graphle/${toWorkspaceKey(issueIdentifier)}`;
}
...
export class CheckoutManager {
  ...
  async #commitWithMessage(cwd: string, subject: string, body?: string) {
    ...
    if (!(await this.#gitConfigValue(cwd, "user.name"))) {
      commitArgs.push("-c", "user.name=Graphle Agent");
    }
  ...
  async #mergeIntoSourceRepo(issue: IssueRuntimeState, streamRef: string) {
    ...
    const mergeBranch = `graphle-merge-${toWorkspaceKey(issue.streamIssueIdentifier)}`;
  ...
  async #squashFeatureBranchIntoBase(
    ...
    const finalizeBranch = `graphle-finalize-${toWorkspaceKey(stream.parentIssueIdentifier)}`;
  ...
  async #mergeTaskCommitIntoBranch(
    ...
    const landingBranch = `graphle-land-${toWorkspaceKey(issue.identifier)}`;
