...
function createWorkerSession(overrides: SessionRefOverrides = {}): AgentSessionRef {
  ...
  return {
    ...
    title: "Run plan",
    workerId: "OPE-68",
    workspacePath: "/Users/dpeek/code/graphle/tmp/workspace/tree/ope-68",
    ...sessionOverrides,
...
test("AgentTuiStore tracks column hierarchy, summaries, and event history", () => {
  ...
  expect(supervisorContent).toContain("Session started | /Users/dpeek/code/graphle");
...
test("AgentTuiStore keeps active workers ahead of a short retained completed and failed tail", () => {
  ...
  const failedWorker = createWorkerSession({
    ...
    title: "Handle failure path",
    workerId: "OPE-69",
    workspacePath: "/Users/dpeek/code/graphle/tmp/workspace/tree/ope-69",
  });
  const activeWorker = createWorkerSession({
    ...
    title: "Keep active work visible",
    workerId: "OPE-70",
    workspacePath: "/Users/dpeek/code/graphle/tmp/workspace/tree/ope-70",
  });
  const recentCompletedWorker = createWorkerSession({
    ...
    title: "Inspect recent completion",
    workerId: "OPE-71",
    workspacePath: "/Users/dpeek/code/graphle/tmp/workspace/tree/ope-71",
  });
...
test("buildAgentTuiRootComponentModel keeps workflow stream, task, blocker, and finalization context inspectable", () => {
  ...
  expect(model.summaryLines).toEqual([
    ...
    "Selected: task OPE-188 | blocked | stream OPE-121 | feature OPE-174 | graphle/ope-174",
  ]);
  ...
  expect(streamContent).toContain("branch: graphle/ope-174");
  ...
  expect(streamContent).toContain(`OPE-174: committed ${commitSha} on graphle/ope-174`);
  expect(streamContent).toContain(
  ...
  expect(blockedContent).toContain("branch: graphle/ope-174");
...
test("buildAgentTuiRootComponentModel highlights completed Linear writes", () => {
        ...
        result: {
          structuredContent: {
            issue: {
              identifier: "OPE-68",
              title: "Run plan",
              url: "https://linear.app/graphle/issue/OPE-68",
            },
          },
        },
        server: "linear",
  ...
  expect(content).toContain("  url: https://linear.app/graphle/issue/OPE-68");
...
test("buildAgentTuiRootComponentModel keeps live transcript slices readable in replayable snapshots", () => {
  ...
  expect(replayContent).toContain("  url: https://linear.app/graphle/issue/OPE-68");
...
test("createAgentTui supports keyboard column navigation and content scrolling", async () => {
  ...
  try {
    ...
    expect(frame).toContain("/Users/dpeek/code/graphle");
