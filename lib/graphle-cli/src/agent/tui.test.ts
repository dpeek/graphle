...
function createSupervisorSession(overrides: SessionRefOverrides = {}): AgentSessionRef {
  ...
  return {
    ...
    title: "Supervisor",
    workerId: "supervisor",
    workspacePath: "/Users/dpeek/code/graphle",
    ...sessionOverrides,
...
function createWorkerSession(overrides: SessionRefOverrides = {}): AgentSessionRef {
  ...
  const baseIssue = {
    id: "issue-67",
    identifier: "OPE-67",
    title: "Implement graphle agent tui",
  };
  return {
    branchName: "ope-67",
    id: "worker:OPE-67:1",
    ...
    title: "Implement graphle agent tui",
    workerId: "OPE-67",
    workspacePath: "/Users/dpeek/code/graphle/tmp/workspace/tree/ope-67",
    ...sessionOverrides,
...
function createIssueState(
  ...
  return {
    branchName: "ope-67",
    controlPath: "/Users/dpeek/code/graphle/tmp/workspace/control",
    issueId: "issue-67",
    issueIdentifier: "OPE-67",
    issueTitle: "Implement graphle agent tui",
    originPath: "/Users/dpeek/code/graphle",
    ...
    workerId: "OPE-67",
    worktreePath: "/Users/dpeek/code/graphle/tmp/workspace/tree/ope-67",
    ...overrides,
  };
}
...
test("AgentTuiStore keeps supervisor first and records status plus raw output", () => {
  ...
  store.observe({
    ...
    sequence: 2,
    session: supervisor,
    text: "Graphle is supervising /Users/dpeek/code/graphle",
  ...
  expect(snapshot.sessions[0]?.body).toContain("Session started | /Users/dpeek/code/graphle");
  expect(snapshot.sessions[0]?.body).toContain("Graphle is supervising /Users/dpeek/code/graphle");
  expect(snapshot.sessions[1]?.body).toContain(
    "Session scheduled | ope-67 | /Users/dpeek/code/graphle/tmp/workspace/tree/ope-67\n",
  );
...
test("renderAgentTuiFrame lays out supervisor and worker columns", () => {
  const sessions = [
    createSnapshotColumn({
      ...
      body:
        "Session started | /Users/dpeek/code/graphle\n" +
        "Graphle is supervising /Users/dpeek/code/graphle\n" +
        "Workflow: idle\n",
      firstSequence: 1,
      lastSequence: 3,
      phase: "started",
...
test("AgentTuiRetainedReader reconstructs events.log into supervisor and worker columns", async () => {
  ...
  try {
    const reader = new AgentTuiRetainedReader({
      issueState,
      repoRoot: "/Users/dpeek/code/graphle",
    });
    const store = createAgentTuiStore();
...
test("AgentTuiRetainedReader keeps workflow blocker context visible in attach mode", async () => {
  ...
  try {
    ...
    expect(snapshot.sessions[0]?.body).toContain(
      "runtime state: blocked; worktree preserved on graphle/ope-174",
    );
    ...
    expect(snapshot.sessions[1]?.body).toContain(
      "Session scheduled | graphle/ope-174 | /repo/tmp/workspace/tree/ope-188",
    );
    expect(snapshot.sessions[1]?.body).toContain("OPE-188: blocked");
    expect(snapshot.sessions[1]?.body).toContain(
      "Session failed | graphle/ope-174 | /repo/tmp/workspace/tree/ope-188: Blocked on OPE-187 finalization",
    );
...
test("AgentTuiRetainedReader keeps finalized workflow context visible in replay mode", async () => {
  ...
  try {
    ...
    expect(snapshot.sessions[1]?.session.branchName).toBe("graphle/ope-174");
    expect(snapshot.sessions[1]?.session.runtime).toMatchObject({
      finalization: {
        linearState: "Done",
        state: "finalized",
    ...
    expect(snapshot.sessions[1]?.phase).toBe("completed");
    expect(snapshot.sessions[1]?.body).toContain(
      "Session completed | graphle/ope-174 | /repo/tmp/workspace/tree/ope-174",
    );
    expect(frame).toContain("Replay OPE-174 from codex.stdout.jsonl");
    expect(frame).toContain("Session completed | graphle/ope-174");
  } finally {
    await rm(root, { force: true, recursive: true });
  }
});
...
test("AgentTuiRetainedReader supplements partial events.log with retained workflow milestones", async () => {
  ...
  try {
    ...
    expect(snapshot.sessions[0]?.body).toContain("landed: abc1234 on graphle/ope-174");
    expect(snapshot.sessions[0]?.body).toContain("finalized: Done");
    expect(snapshot.sessions[1]?.status).toMatchObject({
      ...
      text: `OPE-174: committed ${commitSha} on graphle/ope-174`,
    });
    expect(snapshot.sessions[1]?.body).toContain(
...
test("AgentTuiRetainedReader inserts retained commit context before a partial completed lifecycle", async () => {
  ...
  try {
    ...
    const commitIndex = body.indexOf(`OPE-174: committed ${commitSha} on graphle/ope-174`);
    ...
    expect(snapshot.sessions[1]?.eventHistory.map((entry) => entry.summary).slice(-2)).toEqual([
      `issue-committed: OPE-174: committed ${commitSha} on graphle/ope-174`,
      "Session completed | graphle/ope-174 | /repo/tmp/workspace/tree/ope-174",
    ]);
  } finally {
    await rm(root, { force: true, recursive: true });
