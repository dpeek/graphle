...
async function writeServiceTestRepo(root: string, overrides: Record<string, unknown> = {}) {
  ...
  await writeFile(resolve(root, "graphle.md"), "Issue {{ issue.identifier }}\n");
...
test("normalizeLinearIssue leaves parent and stream fields empty for standalone issues", () => {
  const issue = normalizeLinearIssue({
    ...
    labels: { nodes: [{ name: "graphle" }, { name: "agent" }] },
    parent: {
      id: "   ",
      identifier: "OPE-147",
      ...
      workspaceManagerFactory: (_workflow, issueIdentifier) =>
        createWorkspaceManagerStub({
          ...
          prepare: async () => ({
            ...
            baseBranchName: "graphle/ope-121",
    ...
    const service = new AgentService({
      ...
      workspaceManagerFactory: (_workflow, issueIdentifier) =>
        createWorkspaceManagerStub({
          cleanup: async () => undefined,
          complete: async () => {
            throw new Error("task_landing_rebase_failed:OPE-58:graphle/ope-167:conflict");
          },
          createIdleWorkspace: () => ({
            branchName: "main",
            controlPath: root,
...
test("AgentService eagerly creates worker checkout on start", async () => {
  ...
  try {
    ...
    await service.start();
    expect(writes.some((entry) => entry.includes("Graphle is supervising"))).toBe(true);
...
test("AgentService surfaces workflow diagnostics for retained and skipped task states", async () => {
  ...
  try {
    ...
    expect(diagnosticLines).toContain(
      "Active: stream OPE-121 / feature OPE-167 / task OPE-71 on graphle/ope-167",
    );
    expect(diagnosticLines).toContain(
      "Preserved blocked: stream OPE-121 / feature OPE-169 / task OPE-73 on graphle/ope-169",
    );
    expect(diagnosticLines).toContain(
      "Preserved interrupted: stream OPE-121 / feature OPE-168 / task OPE-72 on graphle/ope-168",
    );
    expect(diagnosticLines).toContain(
      "Waiting on finalization: stream OPE-121 / feature OPE-170 / task OPE-74 on graphle/ope-170",
    );
    expect(diagnosticLines).toContain(
...
test("AgentService composes execute built-ins with graphle.md", async () => {
  const root = await mkdtemp(resolve(tmpdir(), "agent-service-"));
  const ioPromptPath = resolve(root, "graphle.md");
...
test("AgentService uses builtin override files from graphle.ts", async () => {
  const root = await mkdtemp(resolve(tmpdir(), "agent-service-"));
  const overridePath = resolve(root, "graphle", "context", "validation-override.md");
  const ioPromptPath = resolve(root, "graphle.md");
  const localPrompt = "LOCAL EXECUTE {{ issue.identifier }}\n";
  const workspacePath = resolve(root, "workspace", "workers", "OPE-56", "repo");
  let capturedPrompt = "";
  ...
  await writeIoTsConfig(root, {
    ...
    context: {
      overrides: {
        "builtin:graphle.core.validation": "./graphle/context/validation-override.md",
      },
    },
    tracker: {
      apiKey: "$LINEAR_API_KEY",
...
test("AgentService writes unresolved issue doc warnings to the run summary", async () => {
  ...
  await writeFile(resolve(root, "graphle", "context", "linked.md"), "LINKED ISSUE DOC\n");
  process.env.LINEAR_API_KEY = "linear-token";
  process.env.LINEAR_PROJECT_SLUG = "project-slug";
...
- \`./graphle/context/missing.md\``,
  ...
  try {
    ...
    expect(output).toContain("1. builtin:graphle.agent.execute.default [builtin]");
