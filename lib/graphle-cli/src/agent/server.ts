import { createLogger, handleExit } from "@dpeek/utils";
...
function parseRetainedTuiOptions(
  ...
  if (!options.issueIdentifier) {
    throw new Error(
      `Usage: graphle agent tui ${mode} <issue> [entrypointPath]${mode === "replay" ? " [--delay-ms <ms>]" : ""}`,
    );
  }
  return options;
}
  ...
  switch (command) {
    ...
    case "tail": {
      ...
      if (!issueIdentifier || issueIdentifier.startsWith("--")) {
        throw new Error("Usage: graphle agent tail <issue> [entrypointPath]");
      }
