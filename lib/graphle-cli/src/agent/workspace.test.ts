...
test("WorkspaceManager prepares a detached issue worktree with flat runtime state", async () => {
  ...
  try {
    ...
    expect(workspace.branchName).toBe("graphle/ope-43");
...
test("WorkspaceManager coordinates standalone stream finalization through the shared workflow path", async () => {
  ...
  try {
    ...
    expect(await run(["git", "branch", "--list", "graphle/ope-43"], repoRoot)).toBe("");
...
test("WorkspaceManager lands task work onto the latest parent feature branch", async () => {
  ...
  try {
    ...
    await run(["git", "checkout", "graphle/ope-12"], repoRoot);
    ...
    expect(await run(["git", "show", "graphle/ope-12:feature.txt"], repoRoot)).toBe(
      "feature branch advance",
    );
    expect(await run(["git", "show", "graphle/ope-12:task.txt"], repoRoot)).toBe("task change");
...
test("WorkspaceManager preserves task worktree state when landing rebase conflicts", async () => {
  ...
  try {
    ...
    expect(await run(["git", "rev-parse", "graphle/ope-12"], repoRoot)).toBe(featureCommitSha);
