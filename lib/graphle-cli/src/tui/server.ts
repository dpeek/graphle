...
Defaults:
  ...
  graph source: workflow config tui.graph.url, else http://graphle.localhost:1355/
...
export function parseWorkflowTuiCliArgs(args: string[]): WorkflowTuiCliOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    throw new Error("Usage: graphle tui [entrypointPath]");
  }
...
function resolveInitialProjectId(
  ...
  if (visibleProjects.length === 0) {
    throw new Error(
      "Workflow TUI startup could not infer an initial project because the synced workflow scope contains no visible WorkflowProject records. Pass --project or set graphle.ts tui.initialScope.project.",
    );
  }

  throw new Error(
    `Workflow TUI startup could not infer an initial project because the synced workflow scope contains ${visibleProjects.length} visible WorkflowProject records. Pass --project or set graphle.ts tui.initialScope.project.`,
  );
}
