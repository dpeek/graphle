...
import { createWorkflowProjectionIndex } from "@dpeek/graphle-module-workflow";
...
function createWorkflowProjectionFixture() {
  ...
  const repositoryId = graph.repository.create({
    name: "graphle",
    project: projectId,
    repositoryKey: "repo:graphle",
    repoRoot: "/tmp/graphle",
  ...
  graph.repositoryCommit.create({
    ...
    worktree: {
      path: "/tmp/graphle-worktree-runtime",
      branchName: "workflow/runtime-contract",
  ...
  graph.repositoryCommit.create({
    ...
    worktree: {
      path: "/tmp/graphle-worktree-shell",
      branchName: "workflow/shell-polish",
...
test("buildWorkflowTuiRootComponentModel renders the startup loading shell", () => {
  const model = createWorkflowTuiStartupLoadingModel({
    entrypointPath: "/Users/dpeek/code/graphle/graphle.ts",
    workspaceRoot: "/Users/dpeek/code/graphle/tmp/workspace",
  });

  const layout = buildWorkflowTuiRootComponentModel(model);
  expect(layout.summaryLines).toEqual([
    "Graphle Workflow TUI",
  ...
  expect(layout.panels.find((panel) => panel.id === "surface")?.body).toContain(
    "/Users/dpeek/code/graphle/tmp/workspace",
  );
  expect(layout.panels.find((panel) => panel.id === "surface")?.body).toContain(
    "http://graphle.localhost:1355/",
  );
  ...
  expect(layout.footerLines).toContain("Legacy session monitor: graphle agent tui");
});
...
test("buildWorkflowTuiRootComponentModel renders workflow branch board, detail, and commit queue panels", () => {
  ...
  expect(layout.summaryLines).toEqual([
    "Graphle Workflow TUI",
    "Project: Graphle (project:graphle) | Repository: graphle -> main | Focus: Branch board",
...
test("buildWorkflowTuiRootComponentModel renders selected branch detail metadata", () => {
  ...
  expect(branchDetailBody).toContain("Repository: graphle (repo:graphle) -> main");
...
test("buildWorkflowTuiRootComponentModel renders commit queue repository summaries", () => {
  ...
  expect(commitQueueBody).toContain(
    "state attached | branch workflow/runtime-contract | worktree /tmp/graphle-worktree-runtime | lease attached",
  );
...
test("createWorkflowTui hydrates from loading state into the graph-backed workflow shell", async () => {
  ...
  try {
    ...
    expect(frame).toContain("Project: Graphle (project:graphle)");
