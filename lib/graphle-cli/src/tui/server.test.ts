...
function createWorkflow(overrides: Partial<Workflow["tui"]> = {}): Workflow {
  ...
  return {
    ...
    entrypoint: {
      configPath: "/workspace/graphle.ts",
      kind: "graphle",
      promptPath: "/workspace/graphle.md",
    },
    entrypointContent: "# graphle",
    hooks: {
      timeoutMs: 1,
    },
    issues: {
...
test("parseWorkflowTuiCliArgs accepts the default bootstrap command shape", () => {
  ...
  expect(parseWorkflowTuiCliArgs(["./graphle.ts"])).toEqual({
    branchId: undefined,
    graphUrl: undefined,
    help: false,
    projectId: undefined,
...
test("parseWorkflowTuiCliArgs rejects unexpected extra arguments, missing values, or flags", () => {
  expect(() => parseWorkflowTuiCliArgs(["./graphle.ts", "./graphle.md"])).toThrow(
    "Usage: graphle tui [entrypointPath]",
  );
...
test("runWorkflowTuiCli bootstraps the synced workflow runtime before creating the TUI", async () => {
  ...
  try {
    await runWorkflowTuiCli(["./graphle.ts"], {
      createGraphClient,
      createTui,
      handleExit: () => {},
    ...
    if (!surfaceModel || surfaceModel.kind !== "workflow") {
      throw new Error("Expected graphle tui to build a workflow surface model.");
    }
...
test("createWorkflowTuiRuntimeBootstrap requires an explicit project when more than one project is visible", async () => {
  ...
  await expect(
    ...
    "Workflow TUI startup could not infer an initial project because the synced workflow scope contains 2 visible WorkflowProject records. Pass --project or set graphle.ts tui.initialScope.project.",
  );
});
