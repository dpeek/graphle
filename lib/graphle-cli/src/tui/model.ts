...
function buildWorkflowTuiStartupModel(
  ...
  const legacyCommand = options.legacyCommand ?? "graphle agent tui";
  const contract =
    options.contract ??
    createDefaultWorkflowTuiStartupContract({
  ...
  return {
    footerLines: [
      ...
      loading
        ...
        : "Status: startup failed. Review the error below, then quit or rerun graphle tui.",
    ...
    panels: [
      ...
      },
      {
        ...
        lines: loading
          ? [
              "lib/cli/src/tui owns terminal workflow UX and product-shell composition.",
              ...
              "lib/cli/src/agent/tui stays in place as the legacy retained session monitor during migration.",
            ]
          : [
