...
import { validateIncrementalSyncResult } from "@dpeek/graphle-sync";
...
import { createJsonPersistedAuthoritativeGraph } from "@dpeek/graphle-authority/server";
...
async function createTempSnapshotPath(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "graphle-graph-authority-"));
  tempDirs.push(dir);
