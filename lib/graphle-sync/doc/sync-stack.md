...
description: "Cross-package ownership for scopes, payloads, authoritative replay, and retained projections centered on @dpeek/graphle-sync."
...
- the question spans `@dpeek/graphle-kernel`, `@dpeek/graphle-sync`,
  `@dpeek/graphle-authority`, `@dpeek/graphle-client`, `@dpeek/graphle-projection`, or
...
- `@dpeek/graphle-kernel` owns authoritative write-envelope contracts,
  canonicalization, and snapshot-diff helpers.
- `@dpeek/graphle-sync` owns sync scopes, payload contracts, fallback vocabulary,
  cursor helpers, validation, and the transport-neutral total-sync session.
- `@dpeek/graphle-authority` owns authoritative apply, retained-history replay,
  total payload creation, and incremental delivery from one authoritative
  store.
- `@dpeek/graphle-client` owns pending local write replay, `"pushing"` status, and
  the client reconcile layer built on top of sync-core.
- `@dpeek/graphle-projection` owns named module scope contracts, retained
...
`@dpeek/graphle-projection` owns the named module scope definitions and request or
delivered scope helpers. `@dpeek/graphle-sync` owns the payload shape that carries
those scopes.
...
`@dpeek/graphle-sync` owns the fallback vocabulary. `@dpeek/graphle-projection` owns the
...
`@dpeek/graphle-authority` owns authoritative replay generation.
`@dpeek/graphle-sync` owns validation and apply behavior for the shared payloads.

### Session layering

The session split is deliberate:

- `@dpeek/graphle-sync` owns total payload apply, incremental apply, delivered
  state, and recent sync activities
- `@dpeek/graphle-client` layers local pending-write replay and client-only
  `"pushing"` state on top of that total-sync session
- `@dpeek/graphle-authority` owns incremental generation from retained authoritative
  history

Do not move client-local pending queue or push retry behavior into
`@dpeek/graphle-sync`.
Do not move authoritative replay generation into `@dpeek/graphle-client`.
...
- `@dpeek/graphle-projection` owns retained compatibility metadata keyed by
