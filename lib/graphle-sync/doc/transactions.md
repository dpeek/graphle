...
description: "Transaction preparation, canonicalization, snapshot materialization, and store apply behavior in @dpeek/graphle-sync."
...
- `../../lib/graphle-kernel/src/tx.ts`: kernel-owned write-envelope and canonicalization contract

## Boundary

`@dpeek/graphle-sync` does not define the write-envelope contract. That belongs to `@dpeek/graphle-kernel`.
...
- Keep canonical transaction semantics in `@dpeek/graphle-kernel`; do not fork them here.
