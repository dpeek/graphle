...
`@dpeek/graphle-sync` is the extracted sync-core boundary for graph payloads,
...
Kernel-owned write-envelope symbols and `GraphWriteScope` come from `@dpeek/graphle-kernel` directly.
...
- Derive write envelopes from snapshots: `createGraphWriteTransactionFromSnapshots` from `@dpeek/graphle-kernel`
...
- `SyncStatus` in this package is total-sync-only and excludes `@dpeek/graphle-client`'s `"pushing"` phase.
- Diagnostics describe retained-history context only. They do not change apply rules by themselves.

## Public API

`@dpeek/graphle-sync` exposes a single public entrypoint from `./src/index.ts`.
...
Run `turbo build --filter=@dpeek/graphle-sync` from the repo root, or
`bun run build` in this package, to emit `./out`.
Run `turbo check --filter=@dpeek/graphle-sync` from the repo root, or
