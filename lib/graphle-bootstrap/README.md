...
`@dpeek/graphle-bootstrap` owns graph schema bootstrap and materialization.
...
`@dpeek/graphle-bootstrap` only understands icon contracts:
...
Run `turbo build --filter=@dpeek/graphle-bootstrap` from the repo root, or
`bun run build` in this package, to emit `./out`.
Run `turbo check --filter=@dpeek/graphle-bootstrap` from the repo root, or
