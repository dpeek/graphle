...
export function createSavedQueryRepositoryFromGraph(
...
export function createSavedViewDefinitionInput(input: {
  ...
  const validation = validateQueryContainer(
    {
      ...input.spec,
      query: {
        kind: "saved-query",
...
export async function resolveSavedQuery(input: {
  readonly catalog: QueryEditorCatalog;
...
export async function resolveSavedView(input: {
  readonly catalog: QueryEditorCatalog;
  readonly executionContext?: QueryIdentityExecutionContext;
  readonly params?: Readonly<Record<string, QueryLiteral>>;
  readonly query: SavedQueryDefinition;
  ...
  const resolvedQuery = await resolveSavedQuery({
    catalog: input.catalog,
    executionContext: input.executionContext,
...
export function createSavedQuerySourceResolver(
  ...
  return async (source) => {
    ...
    return {
      parameterDefinitions: saved.parameterDefinitions,
      request,
      sourceCacheKey: `saved-query:${saved.id}:${serializeQueryParams(source.params)}`,
    };
  };
}
...
export function validateSavedViewCompatibility(input: {
  ...
  if (surfaceCompatibility && rendererCapabilities) {
    const containerValidation = validateQueryContainer(createSavedViewContainerSpec(view), {
...
function serializeQueryParams(params?: Readonly<Record<string, QueryLiteral>>): string {
  if (!params) {
    return "";
  }
