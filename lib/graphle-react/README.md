...
`@dpeek/graphle-react` is the host-neutral React layer for synced graph clients.
...
- `@dpeek/graphle-react` reads predicate metadata and reference policy, but it does
...
Run `turbo build --filter=@dpeek/graphle-react` from the repo root, or `bun run build`
in this package, to emit `./out`.
Run `turbo check --filter=@dpeek/graphle-react` from the repo root, or
