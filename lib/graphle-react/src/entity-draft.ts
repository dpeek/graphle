...
export type CreateEntityDraftControllerOptions = {
  ...
  readonly validate: (input: Record<string, unknown>) => GraphMutationValidationResult;
};
...
export function createEntityDraftController({
  ...
  typeById,
  validate,
}: CreateEntityDraftControllerOptions): EntityDraftController {
  ...
  function validateMutation(
    ...
    const nextInput = setDraftValue(currentInput, path, fieldName, nextValue);
    const validation = validate(nextInput);
