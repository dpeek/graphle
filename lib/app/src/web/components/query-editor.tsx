                                    ...
                                    filter.id,
                                    {
                                      value:
                                        event.target.value === "param"
                                          ...
                                          : {
                                              kind: "literal",
                                              value: defaultLiteralValue(field, filter.operator),
                                            },
                                    },
                                    catalog,
                                  ),
...
function renderListEditor(
  ...
  const listValue = Array.isArray(value)
    ...
    : [];
  if ((field.control === "enum" || field.control === "entity-ref") && field.options?.length) {
    return (
      <select
...
function ParameterDefaultEditor({
  ...
  switch (parameter.type) {
    ...
    case "entity-ref-list":
      return (
        <Textarea
          data-query-editor-control={parameter.type === "string-list" ? "text" : "entity-ref"}
          onChange={(event) => {
            onChange(
              event.target.value
                .split(",")
...
function defaultLiteralValue(
  field: QueryEditorFieldSpec,
  operator: QueryEditorDraft["filters"][number]["operator"],
): QueryEditorRawValue {
  if (operator === "exists") {
    return true;
  }
