...
describe("workflow projection query helpers", () => {
  it("reads the project branch board with ordering, freshness, unmanaged joins, and pagination", () => {
    ...
    expect(firstPage.project.projectKey).toBe("project:graphle");
    expect(firstPage.repository?.repositoryKey).toBe("repo:graphle");
  ...
  it("reads the implicit main commit workflow with selected-commit detail and retained linkage", () => {
    ...
    expect(result.repository?.repositoryKey).toBe("repo:graphle");
