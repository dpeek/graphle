...
description: "Cross-package ownership for the browser-first v1 workflow contract centered on @dpeek/graphle-module-workflow."
...
- the question spans `@dpeek/graphle-module-workflow`, `@dpeek/graphle-module-core`,
  `@dpeek/graphle-authority`, app-owned workflow routes, or the operator-facing v1
  workflow contract
...
- `@dpeek/graphle-module-workflow` owns the concrete built-in `workflow:` schema,
...
- `@dpeek/graphle-module-core` owns the shared durable `core:` records those
  workflow contracts depend on, especially saved queries, saved views, and
  shared value or identity contracts
- `@dpeek/graphle-authority` owns authoritative workflow execution, installed-module
