...
`@dpeek/graphle-module-workflow` is the canonical workspace package for the built-in
`workflow:` namespace.
...
- `@dpeek/graphle-module` for module authoring helpers
- `@dpeek/graphle-module-core` for built-in core scalar and shared value contracts
- `@dpeek/graphle-kernel` for ids and low-level graph helpers
- `@dpeek/graphle-client` for typed query helpers
- `@dpeek/graphle-projection` for retained projection and invalidation contracts

## What It Does Not Own

- generic helpers from `@dpeek/graphle-module`
- `core:` schema/contracts from `@dpeek/graphle-module-core`
- graph-owned saved-query, saved-query-parameter, and saved-view durability
  records from `@dpeek/graphle-module-core`
- host-neutral React helpers from `@dpeek/graphle-react`
...
- `@dpeek/graphle-module-workflow`
- `@dpeek/graphle-module-workflow/client`
- `@dpeek/graphle-module-workflow/server`
...
```ts
import { workflowManifest, workflowQuerySurfaceCatalog } from "@dpeek/graphle-module-workflow";
```
...
Run `turbo build --filter=@dpeek/graphle-module-workflow` from the repo root, or
`bun run build` in this package, to emit `./out`.
Run `turbo check --filter=@dpeek/graphle-module-workflow` from the repo root, or
