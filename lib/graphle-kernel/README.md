...
`@dpeek/graphle-kernel` is the extracted seed of the smallest graph-kernel boundary.
...
`@dpeek/graphle-kernel` exposes a single public entrypoint from `./src/index.ts`.
...
Run `turbo build --filter=@dpeek/graphle-kernel` from the repo root, or
`bun run build` in this package, to emit `./out`.
Run `turbo check --filter=@dpeek/graphle-kernel` from the repo root, or
