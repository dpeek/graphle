...
description: "Cross-package ownership for kernel storage, module authoring, bootstrap, authority seams, and React/runtime layering centered on @dpeek/graphle-kernel."
...
- the question spans `@dpeek/graphle-kernel`, `@dpeek/graphle-module`,
  `@dpeek/graphle-module-core`, `@dpeek/graphle-bootstrap`, `@dpeek/graphle-authority`,
  `@dpeek/graphle-react`, or the small root `@dpeek/graphle-app/graph` helper layer
...
- `@dpeek/graphle-kernel` owns ids, append-oriented store primitives, schema
...
- `@dpeek/graphle-module` owns the definition-time authoring layer above kernel:
...
- `@dpeek/graphle-module-core` owns the built-in `core:` namespace assembly, core
  ...
  the core module.
- `@dpeek/graphle-bootstrap` owns additive bootstrap into a live store, convergent
  bootstrapped snapshots, bootstrap-facing core-schema requirements, and icon
  seed or resolver contracts.
- `@dpeek/graphle-authority` owns authoritative write sessions, persisted-authority
  contracts, replication filtering, and authority-side validation above the
  kernel and sync boundaries.
- `@dpeek/graphle-react` owns the host-neutral React runtime, edit-session
  contracts, validation issue mapping, resolver primitives, and synced runtime
  hooks.
- `@dpeek/graphle-module-core/react-dom` owns the current default browser and DOM
  layer above `@dpeek/graphle-react`.
- `@dpeek/graphle-app/graph` owns only a small curated helper layer plus private
...
not smuggle higher-level meaning into `@dpeek/graphle-kernel`.

### One authoring flow, several owners

The shipped runtime path is layered on purpose:

1. `@dpeek/graphle-kernel` authors raw schema primitives and ids
2. `@dpeek/graphle-module` adds definition-time type, field, command, surface, and
   manifest contracts
3. built-in module packages such as `@dpeek/graphle-module-core` publish concrete
   authored slices
4. `@dpeek/graphle-bootstrap` materializes those authored definitions into a live
...
`@dpeek/graphle-bootstrap`. If the question is "what is the definition contract," it
...
- `@dpeek/graphle-react` for host-neutral React runtime and edit-session behavior
- `@dpeek/graphle-module-core/react-dom` for the current default browser or DOM
  adapter layer
- host runtime code for route shells, Worker composition, browser chrome, and
  other consumer-owned wiring

The root `@dpeek/graphle-app/graph` surface stays intentionally small. New stable runtime
