...
- `@dpeek/graphle-kernel` for ids, store primitives, schema contracts,
  field-authority metadata, and authoritative write envelopes
- `@dpeek/graphle-module` for definition-time type, field, command, surface, and
  manifest contracts
- `@dpeek/graphle-module-core` and `@dpeek/graphle-module-workflow` for the current
  built-in module packages
- `@dpeek/graphle-bootstrap` for schema materialization and bootstrapped snapshots
- `@dpeek/graphle-client` for typed refs, local CRUD, validation, and synced-client
  composition
- `@dpeek/graphle-sync` for total and incremental whole-graph sync contracts
- `@dpeek/graphle-authority` for authoritative write sessions, persisted authority
  runtime, retained-history validation, and graph-owned authority contracts
- `@dpeek/graphle-projection` for module read scopes, projection metadata, retained
  projection compatibility, and invalidation targeting
- `@dpeek/graphle-react` for host-neutral React hooks, edit-session contracts, and
  resolver primitives
- `@dpeek/graphle-surface` for route-neutral record and collection surface runtime
...
- keep `@dpeek/graphle-app/graph` as a small curated helper layer rather than another
