---
name: App auth store
description: "Current Better Auth store, Worker runtime, and migration path for @dpeek/graphle-app."
...
`turbo dev` runs `@dpeek/graphle-app`.

## Provisioning notes

The committed Wrangler config names the database `graphle-better-auth` and uses the
