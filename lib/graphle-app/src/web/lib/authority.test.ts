...
import { defineSecretField, defineType } from "@dpeek/graphle-module";
import {
  core,
...
import { type AgentSessionAppendRequest, workflow } from "@dpeek/graphle-module-workflow";
import {
...
import { type InvalidationEvent } from "@dpeek/graphle-projection";
import {
  ...
  return {
    session: {
      ...
      retainedSession: {
        ...
        workspacePath: "/tmp/graphle-worktree",
      },
...
describe("web authority", () => {
  ...
  it("enforces the v1 inferred-project and attached-repository limits", async () => {
    ...
    await expect(
      executeWorkflowMutation(authority, authorization, {
        ...
        projectId: project.summary.id,
        title: "graphle-2",
        repositoryKey: "repo:graphle-2",
        repoRoot: "/tmp/graphle-2",
        defaultBaseBranch: "main",
      }),
    ).rejects.toMatchObject({
      code: "invalid-transition",
  ...
  it("reads workflow branch board and commit queue scopes from authoritative graph state", async () => {
    ...
    expect(branchBoard.project.projectKey).toBe("project:graphle");
    expect(branchBoard.repository?.repositoryKey).toBe("repo:graphle");
