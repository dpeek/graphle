...
import type { AuthorizationContext, WebPrincipalSummary } from "@dpeek/graphle-authority";
import {
  createGraphClient,
  createHttpGraphClient,
  defaultHttpSerializedQueryPath,
} from "@dpeek/graphle-client";
import { createGraphStore, type GraphStoreSnapshot } from "@dpeek/graphle-kernel";
...
import { webWorkflowReadPath } from "@dpeek/graphle-module-workflow/client";
...
describe("web worker localhost bootstrap routes", () => {
  it("issues a localhost-only bootstrap credential for the configured local origin", async () => {
    const localAuthEnv = createBetterAuthEnv({
      BETTER_AUTH_URL: "http://graphle.localhost:8787",
    });
    ...
    const response = await handler.fetch(
      new Request(`http://graphle.localhost:8787${localhostBootstrapIssuePath}`, {
        method: "POST",
      }),
      env,
    );
    ...
    expect(
      ...
      redeemOrigin: "http://graphle.localhost:8787",
      syntheticIdentity: {
  ...
  it("redeems a localhost bootstrap credential into a real Better Auth session cookie", async () => {
    ...
    const redeemResponse = await handler.fetch(
      new Request(`http://graphle.localhost:8787${localhostBootstrapRedeemPath}`, {
        method: "POST",
        headers: {
          origin: "http://graphle.localhost:8787",
          "content-type": "application/json",
        },
        body: JSON.stringify({
    ...
    const bootstrapResponse = await handler.fetch(
      new Request(`http://graphle.localhost:8787${webAppBootstrapPath}`, {
        headers: {
...
describe("web worker localhost instant onboarding end to end", () => {
  it("turns one localhost click into a writable graph session", async () => {
    ...
    const syncedClient = await createHttpGraphClient(workerGraph, {
      ...
      url: "http://graphle.localhost:8787",
    });
