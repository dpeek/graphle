...
import type { BrowserAgentRuntimeProbe } from "@dpeek/graphle-cli/browser-agent";
...
describe("workflow review page", () => {
  it("renders the commit-first queue, selected commit, and session feed layout", () => {
    ...
    const html = renderToStaticMarkup(
      <WorkflowReviewSurface
        ...
        readState={readState}
        runtime={createRuntimeState()}
        search={{ project: "project-graphle" }}
  ...
  it("keeps the no-selection state explicit when the scope exposes multiple projects", () => {
    ...
    const html = renderToStaticMarkup(
      <WorkflowReviewSurface
        ...
        runtime={createRuntimeState({
          message:
            "Local browser-agent runtime unavailable. Start `graphle browser-agent` on this machine to enable browser launch and attach.",
          status: "unavailable",
        })}
        search={{}}
    ...
    expect(html).toContain("graphle browser-agent");
  });
  ...
  it("keeps stale commit selections explicit instead of silently swapping to another queue row", () => {
    ...
    const html = renderToStaticMarkup(
      <WorkflowReviewSurface
        ...
        readState={readState}
        runtime={createRuntimeState()}
        search={{ commit: "commit:missing", project: "project-graphle" }}
        startupState={createReadyStartupState()}
      />,
    );
  ...
  it("pins the retained session feed to the requesting session while user review is active", () => {
    ...
    expect(resolveEffectiveWorkflowSearch({ project: "project-graphle" }, selectedCommit)).toEqual({
  ...
  it("surfaces commit-scoped retained session context inside the selected commit detail", () => {
    const html = renderToStaticMarkup(
      <WorkflowReviewSurface
        ...
        readState={createReadyReadState()}
        runtime={createRuntimeState()}
        search={{ commit: "commit-1", project: "project-graphle" }}
