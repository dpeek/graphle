...
function createViewsPageFixture(): ViewsPageFixture {
  ...
  const companyId = graph.company.create({
    ...
    status: approvedStatusId,
    tags: [platformTagId],
    website: new URL("https://graphle.example.com"),
  });
  ...
  const recordId = graph.record.create({
    ...
    website: new URL("https://graphle.example.com/views"),
  });
...
export function ViewsPage() {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto pr-1">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground max-w-3xl text-sm leading-6">
              ...
              disposable proof route for shared predicate renderers, editors, reusable query
              container layouts, and the first generic collection-browser composition.
            </p>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
      ...
      <CollectionBrowserProof key={fixtureVersion} />
