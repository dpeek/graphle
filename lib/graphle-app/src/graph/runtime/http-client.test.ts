...
describe("createHttpGraphClient", () => {
  it("uses the localhost default base url", () => {
    expect(defaultHttpGraphUrl).toBe("http://graphle.localhost:1355/");
  });
  ...
  it("keeps hidden-only cursor advances explicit over http without materializing replicated writes", async () => {
    ...
    expect(requestedUrls).toEqual([
      ...
      `http://graphle.localhost:1355/api/sync?after=${encodeURIComponent(baseCursor)}&scopeKind=graph`,
    ]);
  });
  ...
  it("recovers graph fallback reasons over http with a total refetch", async () => {
    for (const fallback of ["unknown-cursor", "gap", "reset"] as const) {
      ...
      expect(requestedUrls).toEqual([
        "http://graphle.localhost:1355/api/sync?scopeKind=graph",
        "http://graphle.localhost:1355/api/sync?after=server%3A1&scopeKind=graph",
        "http://graphle.localhost:1355/api/sync?scopeKind=graph",
      ]);
    }
  });
