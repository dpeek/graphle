...
import {
  ...
  coerceQueryEditorParameterDefaultValue,
  describeUnsupportedQueryEditorFieldKind,
  getQueryEditorBaseFieldKind,
...
export function getQueryEditorSurfaceUnsupportedFields(
  surface: QueryEditorSurfaceSpec,
): readonly QueryEditorFieldSpec[] {
  return surface.fields.filter((field) => !isQueryEditorFieldKindSupported(field.kind));
}

export function describeQueryEditorSurfaceAuthoringExclusions(
  surface: QueryEditorSurfaceSpec,
): string | undefined {
  const unsupportedFields = getQueryEditorSurfaceUnsupportedFields(surface);
  if (unsupportedFields.length === 0) {
    return undefined;
  }

  const labels = unsupportedFields.map((field) => `"${field.fieldId}" (${field.kind})`);
  return (
    `Query surface "${surface.surfaceId}" exposes unsupported query-authoring field kinds: ` +
    `${labels.join(", ")}. ` +
    "List-valued predicate families are excluded from the first /query authoring surface."
  );
}
...
export function hydrateQueryEditorDraft(input: {
  ...
  if (surfaceExclusionMessage) {
    throw new QueryEditorHydrationError(surfaceExclusionMessage);
  }
...
export function addQueryEditorFilter(
  ...
  const field =
    surface?.fields.find((candidate) => isQueryEditorFieldKindSupported(candidate.kind)) ??
    surface?.fields[0];
  if (!field) {
    return draft;
  }
  ...
  const nextFilter: QueryEditorFilterDraft = {
    ...
    operator,
    value: createQueryEditorDefaultFilterValue(field, operator),
  };
...
function collectQueryEditorIssues(
  ...
  if (surfaceExclusionMessage) {
    issues.push({
      ...
      message: surfaceExclusionMessage,
      path: "draft.surfaceId",
    });
  }
  ...
  for (const [index, filter] of draft.filters.entries()) {
    ...
    const unsupportedKindMessage = describeUnsupportedQueryEditorFieldKind(field.kind);
    if (unsupportedKindMessage) {
      issues.push({
        code: "unsupported-field-kind",
        message: `Field "${field.fieldId}" uses unsupported query-authoring kind "${field.kind}". ${unsupportedKindMessage}`,
        path: `${path}.fieldId`,
      });
      continue;
    }
...
function buildSerializedFilter(
  ...
  return {
    fieldId: filter.fieldId,
    op: operator,
    value: coerceSingleQueryValue(filter.value, field, parameters, field.fieldId, operator),
  };
}
...
function coerceFilterDraftValue(
  ...
  if (filter.operator === "in") {
    if (filter.value.kind === "param") {
      ...
      const listType = listParameterTypeForFieldKind(field.kind);
    ...
    const listValue = coerceLiteralValue(field, filter.operator, filter.value.value, true);
    if (!Array.isArray(listValue)) {
      throw new Error(`"${field.fieldId}" in-filters require a list value.`);
    }
    return listValue.map((entry) => ({ kind: "literal", value: entry }) as const);
  }

  return coerceSingleQueryValue(
    filter.value,
    field,
    parameters,
    path,
    filter.operator as Exclude<QueryFilterOperator, "and" | "or" | "not" | "exists" | "in">,
  );
}
...
function coerceSingleQueryValue(
  ...
  parameters: readonly QueryEditorParameterDraft[],
  path: string,
  operator: Exclude<QueryFilterOperator, "and" | "or" | "not" | "exists" | "in">,
): QueryValue {
  if (value.kind === "param") {
    const parameter = requireParameter(parameters, value.name, path);
    const expectedType = parameterTypeForFieldKind(
      getQueryEditorFieldKindForFilterOperator(field.kind, operator),
    );
  ...
  return {
    kind: "literal",
    value: coerceLiteralValue(field, operator, value.value, false),
  };
}
...
function coerceLiteralValue(
  ...
  return coerceQueryEditorFieldLiteralValue(
    getQueryEditorFieldKindForFilterOperator(field.kind, operator),
    rawValue,
    `"${field.fieldId}"`,
    isList,
...
function coerceParameterDefault(parameter: QueryEditorParameterDraft): QueryLiteral {
  return coerceQueryEditorParameterDefaultValue(
    parameter.type,
...
function parameterTypeForFieldKind(kind: QuerySurfaceFieldKind): QueryParameterType {
...
function listParameterTypeForFieldKind(kind: QuerySurfaceFieldKind): QueryParameterType {
  switch (getQueryEditorBaseFieldKind(kind)) {
    case "text":
      return "string-list";
    case "date":
...
export function createQueryEditorDefaultFilterValue(
  field: QueryEditorFieldSpec,
  operator: QueryFilterOperator,
): QueryEditorValueDraft {
  const kind = getQueryEditorFieldKindForFilterOperator(field.kind, operator);
        ...
        field.options && field.options[0]
          ...
          : kind === "boolean"
            ...
            : kind === "number" || kind === "percent"
              ? [1]
              : [defaultStringLiteralForFieldKind(kind)],
    };
  }
  ...
  switch (kind) {
    ...
    case "range":
    case "rate":
      return { kind: "literal", value: defaultStringLiteralForFieldKind(kind) };
  }
}
    ...
    return {
      ...
      value:
        firstValue?.kind === "param"
          ...
          : {
              kind: "literal",
              value: hydrateLiteralListFilterValue(filter),
            },
    };
  }
...
function defaultStringLiteralForFieldKind(kind: QuerySurfaceFieldKind): string {
  switch (getQueryEditorBaseFieldKind(kind)) {
    case "url":
    ...
    case "quantity":
      return "5 kg";
    case "range":
      return "percent(10%) .. percent(80%)";
    case "rate":
      return "money(125 USD) / duration(1 day)";
    default:
      return "value";
  }
}

function hydrateLiteralListFilterValue(
  filter: Extract<QueryFilter, { readonly op: "in" }>,
): QueryLiteral {
  const literals = filter.values.flatMap((value) => {
  ...
  if (typeof first === "string") {
    if (literals.some((value) => typeof value !== "string")) {
      throw new QueryEditorHydrationError(
        `Query editor cannot hydrate mixed literal list types for "${filter.fieldId}".`,
      );
    }
    return literals.filter((value): value is string => value.length > 0);
  }

  if (typeof first === "number") {
    if (literals.some((value) => typeof value !== "number")) {
      throw new QueryEditorHydrationError(
        `Query editor cannot hydrate mixed literal list types for "${filter.fieldId}".`,
      );
    }
  ...
  if (typeof first === "boolean") {
    if (literals.some((value) => typeof value !== "boolean")) {
      throw new QueryEditorHydrationError(
        `Query editor cannot hydrate mixed literal list types for "${filter.fieldId}".`,
      );
    }
  ...
  throw new QueryEditorHydrationError(
    `Query editor only supports scalar list filters for "${filter.fieldId}".`,
  );
}
