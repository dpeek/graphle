...
description: "core:icon, core:svg, icon seeds, fallback resolution, SVG sanitization, and bootstrap wiring in @dpeek/graphle-module-core."
...
- `@dpeek/graphle-kernel` owns the schema-level icon reference and seed-record
  contracts definitions use
- `@dpeek/graphle-module-core` owns the concrete built-in icon and SVG contracts,
  default seed records, fallback resolution, sanitization, and the default DOM
  renderer
- `@dpeek/graphle-bootstrap` owns icon materialization from caller-supplied seeds
  and resolvers
