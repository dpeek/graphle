...
`@dpeek/graphle-projection` is the shared Branch 3 contract boundary for projection
...
- sync payload/session contracts beyond the scope interop owned by `@dpeek/graphle-sync`
...
- `@dpeek/graphle-sync` owns transport-facing sync scopes and payload/session
...
- `@dpeek/graphle-authority` owns authoritative write orchestration, durable state,
...
`@dpeek/graphle-projection` exposes a single public entrypoint from `./src/index.ts`.
...
Run `turbo build --filter=@dpeek/graphle-projection` from the repo root, or
`bun run build` in this package, to emit `./out`.
Run `turbo check --filter=@dpeek/graphle-projection` from the repo root, or
