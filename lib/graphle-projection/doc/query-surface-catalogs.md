...
description: "Bounded query-surface metadata and validation rules in @dpeek/graphle-projection."
...
- filter operators must be non-empty and valid `@dpeek/graphle-client` operators
- order directions must be valid `@dpeek/graphle-client` directions
- parameter types must be valid `@dpeek/graphle-client` parameter types
