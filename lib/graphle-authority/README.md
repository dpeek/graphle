...
`@dpeek/graphle-authority` is the extracted authority/runtime boundary for shared
...
- graph store, schema, ids, and write-envelope primitives from `@dpeek/graphle-kernel`
- sync session, cursor, and payload core contracts from `@dpeek/graphle-sync`
- client runtime and HTTP/query helpers from `@dpeek/graphle-client`
...
`@dpeek/graphle-authority` exposes a browser-safe root entrypoint from `./src/index.ts`
plus a Node-only `@dpeek/graphle-authority/server` subpath for filesystem-backed
helpers.
...
- server-only persistence helpers from `@dpeek/graphle-authority/server`:
  `createJsonPersistedAuthoritativeGraph`,
  `createJsonPersistedAuthoritativeGraphStorage`

## Build Output

Run `turbo build --filter=@dpeek/graphle-authority` from the repo root, or
`bun run build` in this package, to emit `./out`.
Run `turbo check --filter=@dpeek/graphle-authority` from the repo root, or
`bun run check` in this package, to lint, format, type-check, and execute the
package-local Bun tests.

## Package Boundary

`@dpeek/graphle-authority` intentionally avoids a build-time dependency on
`@dpeek/graphle-module-core`. Package-local typecheck probes use self-contained graph
fixtures so `turbo build` can preserve the acyclic package graph. Cross-package
authority coverage now lives in `@dpeek/graphle-integration`.
