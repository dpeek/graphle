...
description: "Authoritative apply flow, idempotent replay, retained-history windows, and total or incremental sync seams in @dpeek/graphle-authority."
...
- you are touching the contract between `@dpeek/graphle-authority`, `@dpeek/graphle-sync`, and `@dpeek/graphle-kernel`
...
- `getIncrementalSyncResult()` wraps retained changes in `@dpeek/graphle-sync` payload helpers and emits explicit fallback reasons when the cursor no longer fits the retained window.
