...
description: "Cross-package ownership for predicate visibility, command-lowering, authorization, and authoritative execution centered on @dpeek/graphle-authority."
...
- `@dpeek/graphle-kernel` owns field visibility metadata, field-write policy
  literals, and canonical write envelopes
- `@dpeek/graphle-module` owns pure authored command and command-surface
  descriptors, including execution-mode metadata
- `@dpeek/graphle-authority` owns shared authorization, share and admission
...
- `@dpeek/graphle-sync` carries write-scope and payload contracts consumed by
...
`@dpeek/graphle-authority` publishes the shared authority runtime. It does not
