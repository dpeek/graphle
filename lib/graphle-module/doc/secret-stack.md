...
description: "Cross-package ownership for secret handles, secret-field authoring, authority writes, and web-side storage centered on @dpeek/graphle-module."
...
- `@dpeek/graphle-module` owns the shared `defineSecretField(...)` authoring helper
  and the schema metadata that marks a field as secret-backed
- `@dpeek/graphle-module-core` owns the graph-visible `core:secretHandle` type
- consumer packages such as `@dpeek/graphle-module-workflow` own secret-backed field
  consumers like `workflow:envVar`
- `@dpeek/graphle-authority` owns the shared authoritative write boundary consumed by
