...
description: "Cross-package ownership for type-module authoring, built-in modules, manifests, and installed-module lifecycle centered on @dpeek/graphle-module."
...
- the question spans `@dpeek/graphle-module`, `@dpeek/graphle-module-core`,
  `@dpeek/graphle-module-workflow`, or `@dpeek/graphle-authority`
...
- `@dpeek/graphle-module` is the shared authoring package
...
`@dpeek/graphle-module`; concrete built-in contracts belong in the consuming module
packages.
...
1. `@dpeek/graphle-module` validates and freezes the authored manifest
2. installers derive one concrete `InstalledModuleTarget` from that manifest
   plus a `bundleDigest`
3. `@dpeek/graphle-authority` persists `InstalledModuleRecord` with install state,
...
That planner seam belongs in `@dpeek/graphle-authority`, not in the authoring
package.
...
- `@dpeek/graphle-module-core` owns core graph-wide contracts and durable
  graph-native records such as saved queries, views, icons, secret handles, and
  identity or admission records
- `@dpeek/graphle-module-workflow` owns workflow-specific schema, commands,
  projections, reads, env vars, and documents

If a contract is generic across modules, it probably belongs in
`@dpeek/graphle-module`. If it is one shipped built-in namespace slice, it belongs
...
- `@dpeek/graphle-module-core` owns the durable `core:savedQuery`,
  `core:savedQueryParameter`, and `core:savedView` records plus the
  `coreQuerySurfaceCatalog`
- `@dpeek/graphle-module-workflow` owns the workflow query-surface catalog and the
