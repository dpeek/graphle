...
`@dpeek/graphle-module` is the extracted definition-time authoring package for graph
schemas.
...
- `@dpeek/graphle-module` is the package.
...
- schema-authoring primitives re-exported from `@dpeek/graphle-kernel`
...
- `@dpeek/graphle-kernel`: schema primitives such as `defineType(...)`,
  `defineScalar(...)`, and `defineEnum(...)`
- `@dpeek/graphle-module`: module-definition helpers and pure authored contracts
- built-in or future module packages: concrete authored modules such as `core`
  and `workflow`

## Public API

`@dpeek/graphle-module` exposes a single public entrypoint from `./src/index.ts`.
...
state contract, lives in `@dpeek/graphle-authority`.

## Build Output

Run `turbo build --filter=@dpeek/graphle-module` from the repo root, or `bun run build`
in this package, to emit `./out`.
Run `turbo check --filter=@dpeek/graphle-module` from the repo root, or
