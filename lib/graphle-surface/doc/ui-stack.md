...
- the question spans `@dpeek/graphle-client`, `@dpeek/graphle-module`,
  `@dpeek/graphle-react`, `@dpeek/graphle-surface`, `@dpeek/graphle-module-core/react-dom`,
...
- `@dpeek/graphle-client` owns typed refs, predicate-slot subscriptions, nested
  field-group traversal, and synced-client ref ergonomics
- `@dpeek/graphle-module` owns pure authored metadata: field and filter contracts,
...
### Ref semantics stay in `@dpeek/graphle-client`
...
typed handles, it belongs in `@dpeek/graphle-client`.
...
These contracts belong in `@dpeek/graphle-module`. They do not own React hooks, DOM
widgets, route registration, or authoritative execution.

### React layer versus surface runtime

The split between `@dpeek/graphle-react` and `@dpeek/graphle-surface` is deliberate:
...
- `@dpeek/graphle-react` reads that metadata through
