...
description: "Future graph-native surface, editing, and route-composition direction centered on @dpeek/graphle-surface."
...
- `@dpeek/graphle-module` owns pure authored surface and command contracts
- `@dpeek/graphle-react` owns edit-session contracts, validation issues, and
  host-neutral resolver primitives
- `@dpeek/graphle-surface` owns the current route-neutral record and collection
  runtime plus the browser mounts
- `@dpeek/graphle-module-core/react-dom` owns the default field, filter, icon, SVG,
...
- `@dpeek/graphle-react` owns the controller and issue contracts
