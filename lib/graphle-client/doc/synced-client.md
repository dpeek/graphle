...
description: "Pending-write replay, flush and sync semantics, and status widening in @dpeek/graphle-client."
...
- `../src/sync-validation.ts`: client-side validators layered on top of `@dpeek/graphle-sync`
...
- `"pushing"` is client-only and does not exist in `@dpeek/graphle-sync`
...
- Keep `"pushing"` and transport retry policy client-local; they do not belong in `@dpeek/graphle-sync`.
