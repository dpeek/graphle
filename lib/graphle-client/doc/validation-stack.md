...
description: "Cross-package ownership for local mutation validation, authoritative apply validation, and sync-boundary validation centered on @dpeek/graphle-client."
...
- `@dpeek/graphle-client` owns the public validation result surface, local mutation
  precheck, and store-dependent runtime validation for local or browser-facing
  clients
- `@dpeek/graphle-authority` owns authoritative transaction, write-result, and
  total-payload validation against authoritative state
- `@dpeek/graphle-sync` owns sync-payload normalization and incremental-apply
...
`@dpeek/graphle-client` owns the public validation result model reused across local
...
`@dpeek/graphle-sync` owns payload normalization and apply preconditions. It does
