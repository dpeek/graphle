...
`@dpeek/graphle-client` is the extracted graph-client boundary for typed local graph
...
- synced client composition on top of `@dpeek/graphle-sync`
...
`@dpeek/graphle-client` exposes a single public entrypoint from `./src/index.ts`.
...
Run `turbo build --filter=@dpeek/graphle-client` from the repo root, or
`bun run build` in this package, to emit `./out`.
Run `turbo check --filter=@dpeek/graphle-client` from the repo root, or
`bun run check` in this package, to lint, format, type-check, and execute the
package-local Bun tests.

Cross-package client/bootstrap coverage lives in `@dpeek/graphle-integration` so the
