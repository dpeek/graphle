...
function createGraphBackedRepository(ownerId: string) {
  ...
  return {
    graph,
    repository: createSavedQueryRepositoryFromGraph(graph, ownerId),
  };
}
...
describe("saved query repository", () => {
  it("cruds owner-scoped graph-backed saved queries and views", async () => {
    ...
    const otherRepository = createSavedQueryRepositoryFromGraph(graph, otherOwnerId);
  ...
  for (const testCase of [
    ...
    it(`resolves graph-backed saved queries and views from graph-native definitions for ${testCase.surfaceId}`, async () => {
      ...
      const sourceResolver = createSavedQuerySourceResolver(
        {
          getSavedQuery(id) {
      ...
      const resolvedView = await resolveSavedView({
        catalog,
        executionContext: {
          policyFilterVersion: "policy:7",
          principalId: ownerId,
        ...
        query,
        rendererCapabilities,
        resolveSurfaceCompatibility: requireSurfaceCompatibility,
        view,
      });
  ...
  it("validates workflow and core saved queries through one shared catalog and source resolver", async () => {
    ...
    for (const testCase of [
      ...
      const resolved = await resolveSavedQuery({
        catalog,
        executionContext: {
          policyFilterVersion: "policy:7",
          principalId: ownerId,
