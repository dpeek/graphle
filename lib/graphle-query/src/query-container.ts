...
export const queryContainerSourceKindValues = ["saved-query", "inline"] as const;
...
export type QueryRendererBindingSpec =
  | {
...
export type QueryRendererBinding = {
  readonly definition?: QueryRendererBindingSpec;
...
export type QueryContainerSpec = {
  ...
  readonly renderer: QueryRendererBinding;
};
...
export function createInlineQueryContainer(
...
export function createSavedQueryContainer(
  source: {
    readonly params?: Readonly<Record<string, QueryLiteral>>;
    readonly queryId: string;
  },
...
function resolveSourceCacheKey(
  ...
  if (spec.query.kind === "saved-query") {
    return `saved-query:${spec.query.queryId}:${normalizedQueryHash}`;
  }
...
export function validateQueryContainer(
  spec: QueryContainerSpec,
...
export function assertValidQueryContainer(
  spec: QueryContainerSpec,
  options: QueryContainerValidationOptions = {},
): QueryContainerSpec {
  const result = validateQueryContainer(spec, options);
  if (!result.ok) {
