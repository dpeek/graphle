...
const defaultListRendererDefinition = {
  kind: "list",
  item: {},
} as const satisfies QueryRendererBindingSpec;
...
export type QueryRendererComponent = {
  readonly capability: QueryRendererCapability;
  readonly Component: ComponentType<QueryRendererViewProps>;
};

export type QueryRendererRegistry = Readonly<Record<string, QueryRendererComponent>>;
...
export function createDefaultListRendererBinding(
...
export function createDefaultTableRendererBinding(
...
export function createDefaultCardGridRendererBinding(
...
function ListRenderer({ container, result }: QueryRendererViewProps) {
  ...
  return (
    <ul className="grid gap-3" data-query-renderer="default:list">
...
function CardGridRenderer({ container, result }: QueryRendererViewProps) {
  ...
  return (
    <div
      ...
      data-query-renderer="default:card-grid"
    >
      {result.items.map((item) => (
        <Card
...
function TableRenderer({
  ...
  return (
    <div
      ...
      data-query-renderer="default:table"
    >
...
function createRendererCapability(
  ...
  return {
    ...
    supportedSourceKinds: ["saved-query", "inline"],
    supportsEntityId: "optional",
  };
}

export function createQueryRendererRegistry(
  definitions: readonly QueryRendererComponent[],
): QueryRendererRegistry {
  const entries: [string, QueryRendererComponent][] = [];
...
export const builtInQueryRendererRegistry = createQueryRendererRegistry([
  {
    capability: createRendererCapability("default:list"),
    Component: ListRenderer,
  },
  {
    capability: createRendererCapability("default:table"),
    Component: TableRenderer,
  },
  {
    capability: createRendererCapability("default:card-grid"),
    Component: CardGridRenderer,
  },
] as const satisfies readonly QueryRendererComponent[]);
