...
import type { QueryContainerRuntimeValue, QueryContainerSpec } from "../query-container.js";
...
const baseSpec = {
  ...
  renderer: createDefaultListRendererBinding({
...
const tableSpec = {
  ...baseSpec,
  renderer: createDefaultTableRendererBinding([
    {
      fieldId: "title",
      label: "Title",
    },
...
function installDom(): {
  ...
  const dom = new JSDOM("<!doctype html><html><body></body></html>", {
    url: "http://graphle.localhost:8787/query",
  });
...
describe("query container surface", () => {
  ...
  it("renders shared stale and refreshing chrome around renderer output", () => {
    ...
    expect(staleHtml).toContain('data-query-renderer="default:list"');
  ...
  it("renders shared error chrome instead of invoking the renderer", () => {
    ...
    expect(html).not.toContain('data-query-renderer="default:list"');
  });
