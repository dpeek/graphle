...
import { serializedQueryVersion, type QueryResultPage } from "@dpeek/graphle-client";
...
describe("query container validation", () => {
  ...
  it("accepts a valid query container spec", () => {
    expect(
      validateQueryContainer(validSpec, {
  ...
  it("rejects invalid container settings and malformed inline queries", () => {
    const result = validateQueryContainer({
      ...validSpec,
      containerId: "   ",
      pagination: {
        mode: "paged",
  ...
  it("throws a dedicated error from assertValidQueryContainer", () => {
    expect(() =>
      assertValidQueryContainer(
        {
          ...validSpec,
...
describe("query container runtime", () => {
  ...
  it("restarts from the first page when saved-query identity changes", async () => {
    ...
    const runtime = createQueryContainerRuntime({
      ...
      async resolveSource(source) {
        const owner =
          source.kind === "saved-query" && typeof source.params?.owner === "string"
            ? source.params.owner
            : "person:avery";
...
describe("@dpeek/graphle-query", () => {
  it("exports the shared query container contract surface", async () => {
    const queryContainerExports = await import("@dpeek/graphle-query");
