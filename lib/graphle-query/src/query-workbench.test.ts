...
import { coreBuiltInQuerySurfaceIds, coreQuerySurfaceCatalog } from "@dpeek/graphle-module-core";
import { workflowQuerySurfaceCatalog } from "@dpeek/graphle-module-workflow";
import { serializedQueryVersion, type SerializedQueryRequest } from "@dpeek/graphle-client";
...
describe("query workbench route state", () => {
  it("round-trips draft preview requests and parameter definitions through route state", () => {
    ...
    const parameterDefinitions = [
      {
        defaultValue: "workflow-project:graphle",
        label: "Project",
        name: "project",
        required: false,
        type: "string",
...
describe("query workbench preview runtime", () => {
  it("executes inline draft previews through the shared runtime path", async () => {
    ...
    const request = {
      params: {
        project: "workflow-project:graphle",
      },
      query: {
        filter: {
          fieldId: "projectId",
