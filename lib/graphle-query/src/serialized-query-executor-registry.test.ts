...
import { coreBuiltInQuerySurfaceIds } from "@dpeek/graphle-module-core";
...
import { workflowBuiltInQuerySurfaceIds } from "@dpeek/graphle-module-workflow";
...
const installedModuleQuerySurfaceRegistry = createInstalledQuerySurfaceRegistry([
...
function requireInstalledSurface(surfaceId: string): InstalledModuleQuerySurface {
  const surface = getInstalledQuerySurface(installedModuleQuerySurfaceRegistry, surfaceId);
  if (!surface) {
...
describe("serialized query executor registry", () => {
  it("resolves collection executors from installed query surfaces", () => {
    ...
    const registry = createQueryExecutorRegistry(installedModuleQuerySurfaceRegistry, [
      {
  ...
  it("fails closed for unregistered, missing, and stale collection executors", () => {
    ...
    const unregistered = resolveCollectionQueryExecutor(
      createQueryExecutorRegistry(installedModuleQuerySurfaceRegistry, []),
      {
        kind: "collection",
    ...
    const missing = resolveCollectionQueryExecutor(
      createQueryExecutorRegistry(installedModuleQuerySurfaceRegistry, []),
      {
        kind: "collection",
        indexId: surface.surfaceId,
    ...
    const stale = resolveCollectionQueryExecutor(
      createQueryExecutorRegistry(installedModuleQuerySurfaceRegistry, [
        {
          queryKind: "collection",
          surfaceId: surface.surfaceId,
  ...
  it("resolves scope executors from registered scope ids and inline module definitions across modules", () => {
    ...
    const byScopeId = resolveScopeQueryExecutor(registry, {
      kind: "scope",
    ...
    const byWorkflowDefinition = resolveScopeQueryExecutor(registry, {
      kind: "scope",
      definition: {
        kind: "module",
    ...
    const byCoreDefinition = resolveScopeQueryExecutor(registry, {
      kind: "scope",
      definition: {
        kind: "module",
  ...
  it("fails closed for ambiguous scope registrations and duplicate executors", () => {
    ...
    const ambiguous = resolveScopeQueryExecutor(
      createQueryExecutorRegistry(ambiguousRegistry, []),
      {
        kind: "scope",
        definition: {
          kind: "module",
