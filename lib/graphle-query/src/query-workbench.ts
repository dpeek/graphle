...
const defaultQueryWorkbenchStorageKey = "graphle.web.query-workbench";
...
export function encodeQueryWorkbenchParameterOverrides(
  overrides: Readonly<Record<string, QueryLiteral>>,
): string {
  return encodeWorkbenchValue(overrides);
}

export function decodeQueryWorkbenchParameterOverrides(
  value: string,
...
export function createQueryWorkbenchSourceResolver(
  ...
  return createSavedQuerySourceResolver(
    {
      getSavedQuery(id) {
        return store.getQuery(id);
      },
