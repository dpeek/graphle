...
description: "Cross-package ownership for transport, installed query surfaces, durable saved queries, and execution boundaries centered on @dpeek/graphle-query."
...
- the question spans `@dpeek/graphle-client`, `@dpeek/graphle-query`,
  `@dpeek/graphle-module-core`, `@dpeek/graphle-module-workflow`,
  `@dpeek/graphle-projection`, or app-owned query wiring
...
- `@dpeek/graphle-client` owns the transport-safe serialized-query contract, HTTP
  graph client helpers, and browser-safe request or response validators.
- `@dpeek/graphle-query` owns the installed query-surface registry, query editor
...
- `@dpeek/graphle-module-core` owns the durable graph-native saved-query records:
...
- `@dpeek/graphle-module-workflow` owns workflow-local query surfaces, workflow
...
- `@dpeek/graphle-projection` owns named module read-scope contracts, retained
  projection compatibility metadata, provider registries, dependency keys, and
  invalidation event contracts.
- `@dpeek/graphle-authority` and host runtime code own authoritative query
...
Transport-safe request or response JSON belongs to `@dpeek/graphle-client`.
...
Durable saved-query identity belongs to `@dpeek/graphle-module-core`. Installed
surface compatibility checks and saved-source resolution belong to
`@dpeek/graphle-query`.

### Installed surfaces and executor compatibility

Installed query surfaces become one runtime registry through `@dpeek/graphle-query`.
...
`@dpeek/graphle-query` owns the reusable query-container runtime above serialized
...
- `@dpeek/graphle-module-core` owns the durable saved-query records and the core
  catalog scope
- `@dpeek/graphle-module-workflow` owns the workflow review scope and workflow
  projection-backed surfaces
- `@dpeek/graphle-projection` owns shared scope, retained-state, and invalidation
