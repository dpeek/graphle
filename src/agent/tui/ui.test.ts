...
test("buildAgentTuiRootComponentModel renders a single human-readable block stream", () => {
  ...
  store.observe({
    ...
    sequence: 5,
    session: worker,
    text: "|  M src/agent/runner/codex.ts",
  ...
  expect(content).toContain("   M src/agent/runner/codex.ts");
  ...
  expect(snapshotColumn?.blocks.find((entry) => entry.kind === "command")).toMatchObject({
    kind: "command",
    outputLines: ["## main", " M src/agent/runner/codex.ts"],
  });
});
...
test("buildAgentTuiRootComponentModel keeps live transcript slices readable in replayable snapshots", () => {
  ...
  store.observe({
    ...
    params: {
      delta: "src/agent/runner/codex.ts | 4 ++--\nsrc/agent/tui/ui.test.ts | 8 ++++++--\n",
      itemId: "cmd-1",
    },
    sequence: 3,
    session: worker,
  ...
  expect(liveContent).toContain("  src/agent/runner/codex.ts | 4 ++--");
