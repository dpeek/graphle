...
test("resolveIssueContext assembles the graph module bundle and keeps refs within graph scope", async () => {
  ...
  const promptPath = resolve(root, "io.md");
  await mkdir(resolve(root, "agent", "doc"), { recursive: true });
  await mkdir(resolve(root, "io", "graph"), { recursive: true });
  await mkdir(resolve(root, "io", "project"), { recursive: true });
  await writeFile(promptPath, "LOCAL {{ selection.agent }} {{ selection.profile }}\n");
  await writeFile(resolve(root, "io", "graph", "overview.md"), "GRAPH IO OVERVIEW DOC\n");
  await writeFile(resolve(root, "io", "graph", "architecture.md"), "GRAPH ARCHITECTURE DOC\n");
  await writeFile(resolve(root, "io", "graph", "linked.md"), "GRAPH LINKED DOC\n");
  await writeFile(resolve(root, "io", "project", "shared.md"), "SHARED DOC\n");
  ...
  try {
    ...
    workflow.modules = {
      graph: {
        allowedSharedPaths: [resolve(root, "io", "project"), resolve(root, "io", "graph")],
        docs: ["./io/graph/overview.md", "./io/graph/overview.md", "./io/graph/architecture.md"],
        id: "graph",
...
`),
    ...
    expect(resolved.bundle.docs.map((doc) => doc.id)).toEqual([
      ...
      "builtin:io.core.git-safety",
      "context.entrypoint",
      "./io/graph/overview.md",
      "./io/graph/architecture.md",
      "./io/graph/linked.md",
      "./io/project/shared.md",
      "issue.context",
    ]);
