...
`../../src/graph/graph/type-module.ts` already defines a real type-module authoring
...
`../../src/graph/graph/contracts.ts` and are exported from the root `@io/core/graph`
surface.

## Root-Safe Authoring Contracts

The newer contract symbols are current package surface, not proposal text.
`../../src/graph/graph/contracts.typecheck.ts` shows the intended usage in real code.
...
Canonical built-in scalar families belong under `../../src/graph/schema/core/*`, with
`../../src/graph/type/*` preserved as thin compatibility re-exports while the migration
...
- migrated core modules:
  ...
  - `../../src/graph/schema/core/email/`
- remaining compatibility-backed scalar modules:
  - `../../src/graph/type/string/`
  - `../../src/graph/type/number/`
  - `../../src/graph/type/boolean/`

Enum families already have a default module path via `../../src/graph/type/enum-module.ts`.
...
- `../../src/graph/schema/core/` for `core:` types
- `../../src/graph/schema/app/<slice>/` for `app:` types
