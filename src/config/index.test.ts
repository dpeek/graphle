...
test("@io/core/config re-exports the repo root config", () => {
  expect(config.install?.brews).toContain("ripgrep");
});

test("@io/core/config exposes the repo context bundle and routing defaults", () => {
  expect(config.context?.entrypoint).toBe("./io/project/overview.md");
...
test("repo workflow docs point backlog work at the three-level stream model", () => {
  const backlog = readFileSync(resolve(repoRoot, "./io/project/backlog.md"), "utf8");
  const goals = readFileSync(resolve(repoRoot, "./io/project/goals.md"), "utf8");
  const overview = readFileSync(resolve(repoRoot, "./io/project/overview.md"), "utf8");
  const workflow = readFileSync(resolve(repoRoot, "./io/project/workflow.md"), "utf8");
  const agentOverview = readFileSync(resolve(repoRoot, "./io/agent/overview.md"), "utf8");
  const workflowPlan = readFileSync(
    resolve(repoRoot, "./io/agent/module-stream-workflow-plan.md"),
    "utf8",
  );
  ...
  expect(agentOverview).toContain("../project/workflow.md");
  ...
  expect(workflowPlan).toContain("../project/workflow.md");
