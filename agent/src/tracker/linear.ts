...
import type { AgentIssue, ManagedCommentChildMutation, TrackerConfig } from "../types.js";
...
type ManagedCommentResult = "blocked" | "noop" | "partial" | "updated";

interface ManagedCommentTrigger {
  commentId: string;
  issue: AgentIssue;
  payload?: {
    docs?: string[];
...
interface ManagedCommentMutation {
...
interface ManagedCommentMutationResult {
...
function renderManagedCommentReply(options: {
  command: string;
  issueIdentifier: string;
  lines: string[];
  result: ManagedCommentResult;
...
export class LinearTrackerAdapter {
  ...
  async applyManagedCommentMutation(
    ...
    const dryRun = mutation.comment.payload?.dryRun ?? false;
    if (!teamId) {
    ...
    if (!replyPayload.success || !replyPayload.commentId) {
