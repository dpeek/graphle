...
- Keep `@io/config` as the stable package boundary for importing the repo-root `io.ts` config from workspace packages.
...
- `@io/config` should stay boring: it re-exports the root config and validates that the boundary works.
